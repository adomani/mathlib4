...
abbrev Correspondence := Std.HashMap String.Pos.Raw PPref × Syntax
...
def generateCorrespondence {m} [Monad m] [MonadLog m] [AddMessageContext m] [MonadOptions m]
  ...
  | (corr, _), k, .ident info rawVal _val _pre, str => do
    ...
    pure (
      ...
      (corr.alter ((info.getTrailing?.getD default).startPos) fun _ =>
        PPref.mk ppEndPos (k.push (.str `ident rawVal.toString))), .ident info.removeSpaces rawVal _val _pre)
  | (corr, _), k, .atom info val, str => do
    let ppEndPos ← atomOrIdentEndPos verbose? (k.push (.str `atom val)) val.toRawSubstring str
    pure (
      {str with startPos := ppEndPos},
      -- Is `getD default` a good idea?  It resolves some panics, but there may be a better default
      (corr.alter ((info.getTrailing?.getD default).startPos) fun _ =>
        PPref.mk ppEndPos (k.push (.str `atom val))), .atom info.removeSpaces val)
  | (corr, stx), k, .node _info kind args, str => do
    (getChoiceNode kind args).foldlM (init := (str, corr, stx)) fun (str, corr, sx) arg => do
      generateCorrespondence verbose? (corr, .node _info.removeSpaces kind (args.map (·.eraseLeadTrailSpaces))) (k.push kind) arg str
  | corr, _, _stx, str => do
    pure (str, corr)
...
def whitespaceLinter : Linter where run := withSetOptionIn fun stx ↦ do
  ...
  if let some pretty := ← Mathlib.Linter.pretty stxNoSpaces then
    ...
    let (_, corr, stxNoSpaces) ← generateCorrespondence true (∅, .missing) #[] stx pp
...
elab "#show_corr " cmd:command : command => do
  ...
  if let some pretty := ← Mathlib.Linter.pretty stxNoSpaces then
    let pp := pretty.toRawSubstring
    let (_, corr, stxNoSpaces) ← generateCorrespondence true (∅, .missing) #[] cmd pretty.toRawSubstring
